...
const Navbar = ({ user, onClick, onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // Optionally, parse and set user state from local storage
  ...
  const renderCompanyLinks = () => {
    ...
    return (
      <>
        ...
        <Link to="/company/${company._id}/applications" className="text-gray-700 hover:text-blue-500">Applications</Link>
...
Navbar.propTypes = {
  user: PropTypes.shape({
    ...
    role: PropTypes.string, // Ensure 'role' is included
  }),
